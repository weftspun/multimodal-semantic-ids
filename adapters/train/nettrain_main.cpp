...
// Engine-free real-dim training loop by tiled orchestration.  Composes the verified
// layer forward/backward (from encbwd) across embed → STACK encoder layers → two
// TPM heads, with residual-split gradient accumulation, then per-tensor general Adam.
// Looped over a fixed (x, target); check_nettrain.py compares the loss curve to a
// torch Adam run from the same init.  Only verified kernels are used; the √D embed
// scale and 1/S head mean are host glue between GPU dispatches.
//   nettrain.exe <gemm><lin><ln><attn><ew><adamn> S NIN D H F NOUT STACK STEPS LR
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char **argv) {
	if (argc < 16) {
		fprintf(
		    stderr,
		    "usage: nettrain <gemm><lin><ln><attn><ew><adamn> S NIN D H F NOUT STACK STEPS LR\n");
		return 2;
	}
	S = atoi(argv[7]);
	NIN = atoi(argv[8]);
	...
	std::vector<float> in(S * NIN + NW + 2 * NOUT);
	FILE *fp = fopen("init.bin", "rb");
	if (!fp || fread(in.data(), sizeof(float), in.size(), fp) != in.size()) {
		fprintf(stderr, "nettrain: cannot read init.bin\n");
		return 1;
	}
	fclose(fp);
	...
	std::vector<float> target(2 * NOUT);
	memcpy(target.data(), p, (size_t)2 * NOUT * sizeof(float));

	// adam registry: G/M/V per weight tensor
	struct AW {
		VkcBuf W, G, M, V;
		int n;
	};
	...
	// caches + layer-io + head buffers + scratch
	auto mkC = [&]() {
		Cache c;
		c.x2 = sc(S * D);
		c.Q = sc(S * D);
	...
	for (int step = 1; step <= STEPS; step++) {
		...
		auto headFwd = [&](Cache &c, LW &w, VkcBuf Y, VkcBuf sum, VkcBuf meanv, VkcBuf Map,
		                   VkcBuf MapB, VkcBuf o) {
			layerFwd(c, w, Xs[STACK], Y);
			dN(S, D, 0, 0);
			R(&csum, {Y, sum, g_dims}, (D + 63) / 64, 1);
		...
		for (int j = 0; j < NOUT; j++) {
			float eg = ogm[j] - target[j], el = olm[j] - target[NOUT + j];
			loss += eg * eg + el * el;
			dogm[j] = eg / NOUT;
			dolm[j] = el / NOUT;
		}
		losses[step - 1] = loss / (2 * NOUT);
		...
		// embed bwd: X0 = √D·(xin·Weᵀ+be) → scale dX0 by √D
		{
	...
	fclose(o);
	printf("nettrain %d steps; loss %.6f -> %.6f\n", STEPS, losses[0], losses[STEPS - 1]);
	return 0;
}
